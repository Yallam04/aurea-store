...
// === NEW: We created a mini-component just for the Auth buttons ===
// Because this loads INSIDE the ClerkProvider below, useAuth will work perfectly!
function SidebarAuthButtons() {
  const { isSignedIn, isLoaded } = useAuth();

  return (
    <div style={{ marginTop: "auto", borderTop: "1px solid rgba(201,168,76,0.2)", paddingTop: "20px" }}>
      {/* What the user sees when LOGGED OUT */}
      {isLoaded && !isSignedIn && (
      ...
      {isLoaded && isSignedIn && (
        <div style={{ padding: "14px 40px", display: "flex", alignItems: "center", gap: "16px" }}>
          <UserButton />
          <span style={{ fontSize: "12px", color: "var(--mist)", letterSpacing: "0.2em", textTransform: "uppercase" }}>
            My Account
          </span>
        </div>
      )}
...
export default function RootLayout({ children }: { children: React.ReactNode }) {
  ...
  // Connect to the cart and calculate the total number of items
        ...
        <body>
          ...
          <aside className={`sidebar ${menuOpen ? "open" : ""}`}>
            ...
            <nav className="sidebar-nav">
              ...
              {/* Injected our mini Auth Component here! */}
              <SidebarAuthButtons />
