...
const FILTERS = ["All", "Gold", "Silver", "Chains", "Rings", "Bracelets", "Pendants"];
...
export default function ProductGrid({ inventory }: { inventory: any[] }) {
  ...
  const filteredInventory = inventory.filter((item) => {
    ...
    if (activeFilter === "Silver") return item.material.includes("Silver") || item.material.includes("Steel");
    if (activeFilter === "Chains") return item.name.includes("Chain");
    if (activeFilter === "Rings") return item.name.includes("Ring") || item.name.includes("Signet");
    if (activeFilter === "Bracelets") return item.name.includes("Bracelet") || item.name.includes("Cuff");
    if (activeFilter === "Pendants") return item.name.includes("Pendant") || item.name.includes("Tag");
    return true;
  });
  ...
  return (
    <>
      <div style={{ background: "var(--charcoal)", padding: "20px 60px", borderBottom: "1px solid rgba(201,168,76,0.15)", display: "flex", gap: "32px", alignItems: "center", flexWrap: "wrap" }}>
        ...
        {FILTERS.map((f) => (
          <button
            ...
            style={{
              background: activeFilter === f ? "var(--gold)" : "none",
              color: activeFilter === f ? "var(--charcoal)" : "var(--stone)",
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: "3px", background: "#111", padding: "3px" }}>
        {visibleItems.map((p) => (
          <div key={p.id} className="product-card enhanced-hover" style={{ background: p.bg_color }} onClick={() => setSelectedProduct(p)}>
            ...
            <div className="product-label" style={{ background: p.bg_color }}>
              ...
              <p style={{ color: "var(--gold)" }}>{p.material} · EGP {p.price}</p>
            </div>
          </div>
        ))}
      </div>

      <section style={{ textAlign: "center", padding: "60px", background: "var(--charcoal)" }}>
        ...
        {visibleCount < filteredInventory.length && (
          <button className="btn-outline-light" onClick={() => setVisibleCount(visibleCount + 10)}>Load More</button>
        )}
      </section>
      ...
      <style jsx>{`
        ...
        .enhanced-hover:hover { transform: translateY(-8px); box-shadow: 0 12px 30px rgba(0,0,0,0.5); z-index: 10; }
        .modal-backdrop { position: fixed; inset: 0; background: rgba(15, 14, 12, 0.85); backdrop-filter: blur(8px); z-index: 9999; display: flex; align-items: center; justify-content: center; padding: 20px; animation: fadeIn 0.3s ease; }
        .modal-content { background: var(--cream); width: 100%; max-width: 900px; height: 600px; display: flex; position: relative; box-shadow: 0 24px 60px rgba(0,0,0,0.4); animation: slideUp 0.4s cubic-bezier(0.16, 1, 0.3, 1); }
        ...
        .toast-notification { position: fixed; bottom: 40px; left: 50%; transform: translateX(-50%); background: var(--cream); color: var(--charcoal); padding: 16px 32px; border-radius: 4px; font-size: 11px; letter-spacing: 0.2em; text-transform: uppercase; z-index: 10000; box-shadow: 0 10px 30px rgba(0,0,0,0.5); border: 1px solid rgba(201,168,76,0.5); animation: toastSlideUp 0.4s cubic-bezier(0.16, 1, 0.3, 1); }
